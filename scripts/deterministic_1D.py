# This one contains the parameters tuned on gtzan dataset
...
def densities2(observations, observation_model, state_model):
    ...
    if len(np.shape(observation_model.pointers)) != 2:  # B or N
        new_obs[np.argwhere(
            observation_model.pointers == 2)] = max(observations[1], 0.03)  # * np.min(state_model.state_intervals) / state_model.state_intervals[np.argwhere(observation_model.pointers == 2)]
        new_obs[np.argwhere(
            observation_model.pointers == 0)] = 0.03#observations[0]  # ((1-alpha) * densities[2] * np.min(state_model.state_intervals) / state_model.state_intervals[np.argwhere(observation_model.pointers == 2)])
    elif len(np.shape(observation_model.pointers)) == 2:  # G
        new_obs = observation_model.pointers[
                      0] * observations  # observation_model.pointers[0] = downbeat weigths   observation_model.pointers[1] = beat weigths
        new_obs[new_obs < 0.005] = 0.03
...
class deterministic_1D:
    ...
    def process(self, activations, **kwargs):
        ...
        for i in range(len(activations)):  # loop through all frames to infer beats/downbeats
            ...
            # if activations[i] <= 0.2:  # only shift
            #     beat_particles += 1
            #     beat_particles[beat_particles > self.st.last_states[0]] = 0
            # else:
            local_beat = ''
            ...
            if activations[i] > 0.4:  # resampling is done only when there is a meaningful activation
                ...
                beat_jump_rewards[:self.st.min_interval] = 0
                ...
                beat_jump_rewards1[:self.st.min_interval] = 0
                ...
                self.st.jump_weights[:self.st.min_interval] = 0
                beat_max = np.argmax(beat_distribution)


            #  downbeat detection
            if (beat_max < (
                    int(.07 / T)) + 1) and (counter * T+4) - path[-1][0] > .45 * T * self.st.min_interval:
                local_beat = 'NoooOOoooW!'

                #   downbeat transition   (motion)
                if np.max(self.st2.jump_weights) > 1:
                    self.st2.jump_weights = 0.7 * self.st2.jump_weights / np.max(self.st2.jump_weights)
                d_weight = self.st2.jump_weights.copy()
                down_jump_rewards1 = - down_distribution * d_weight
                d_weight[d_weight < 0.7] = 0
